...
class Config:
    ...
    # Paths
    parent_out_dir = r"/mnt/g/data/PhD Projects/SR" #r"/scratch/p317470/SRHerschel500/data/processed" # r"/scratch/p317470/SRHerschel500/data/processed" #r"/scratch-shared/dkoopmans" #r"E:\SRHerschel500\DATA" # Output directory of Data, change if needed to
    ...
    # How many different noise realisation per cutout
    n_noise_realisation = 3
...
class ProcessDataSet():
    ...
    def run(self):
        ...
        for idx, cl in enumerate(Config.class_names):
            ...
            img_ID_iter = 0
            for i in range(0, Config.n_noise_realisation):
                datamap = hdu[0].data.copy() if not Config.include_noise else self.add_noise(Config.instr_noise_l[idx], hdu[0].data)
                if Config.class_types[idx] != "target":
                    interp_wcs, interp_datamap = self.interp_datamaps(datamap, hdu[0].header, Config.interp_pixscale_l[idx])
                    size = Config.input_cutout_dims
                else:
                    # We do not have to interpolate the target class
                    interp_wcs = WCS(hdu[0].header)
                    interp_datamap = datamap
                    size = Config.target_cutout_dims

                # Create a figure cutout overlay, to verify correct Training/Val/Test split for each class
                # Only do this for the first datamap
                # Validation and Testing files are only saved for the first datamap 
                # img_ID_iter tracks the file ID number for saving FITS files for the training data.
                fig_cutout_overlay, ax = self.create_cutout_projection_figure(interp_datamap, interp_wcs, idx)
                img_ID_iter = self.generate_cutouts(size, interp_datamap, interp_wcs, cl, Config.class_types[idx], Config.fwhm_l[idx], Config.interp_pixscale_l[idx], "Training", ax, img_ID_iter = img_ID_iter)
                
                if i == 0:
                    # Only the first datamap is used for validation and Testing
                    _ = self.generate_cutouts(size, interp_datamap, interp_wcs, cl, Config.class_types[idx], Config.fwhm_l[idx], Config.interp_pixscale_l[idx], "Validation", ax, img_ID_iter = 0)
                    _ = self.generate_cutouts(size, interp_datamap, interp_wcs, cl, Config.class_types[idx], Config.fwhm_l[idx], Config.interp_pixscale_l[idx], "Test", ax, img_ID_iter = 0)
            
            hdu.close() # Close the HDU to free up memory

            # This is used for a Sample Mosaic plot showing pre-processing vs Post-processing
            self.fov_interp_cutout_list.append(Cutout2D(interp_datamap, position=self.cutout_center_l[0], size=size, wcs=interp_wcs, mode="strict"))
...
if __name__ == "__main__":
    for i, prefix in tqdm(enumerate(prefixes), desc="Processing...", total=len(prefixes)):
        ...
        del dataset # Delete the object to free up memory
        gc.collect()