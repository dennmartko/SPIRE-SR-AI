...
val_ds, val_num_batches = create_dataset_tf(VAL_DIR, input_class_names, target_class_names, BATCH_SIZE*2, is_training=False) # Inference batch_size can always be much larger
...
def train_step(x, y, masks, alpha, model, optimizer):
    ...
    # grads, _ = tf.clip_by_global_norm(grads, 7.5)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    return train_loss, tf.linalg.global_norm(grads)

@tf.function(jit_compile=True)
def train_step_clip(x, y, masks, alpha, model, optimizer):
    with tf.GradientTape() as tape:
        predictions = model(x, training=True)
        train_loss = non_adversarial_loss(predictions, y, masks, alpha)
    grads = tape.gradient(train_loss, model.trainable_variables)
...
for epoch in tqdm(epochs, desc="Training model..."):
    ...
    for idx, batch in enumerate(train_ds):
        ...
        if model_name == "SwinUnet":
            train_batch_loss, batch_norm = train_step_clip(batch_x, batch_y[:, :, :, :n_targets], batch_y[:, :, :, n_targets:], alpha, model, optimizer)
        else:
